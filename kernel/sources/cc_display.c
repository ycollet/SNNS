...
krui_err cc_calculateNetParameters() {
    ...
    FOR_ALL_UNITS(unitPtr) { /* calculate the dimensions of the actual net */
        ...
        if(IS_INPUT_UNIT(unitPtr) && UNIT_IN_USE(unitPtr)) {
            NoOfInputUnits++;
            inputYMax = MAX(y,inputYMax);
            inputYMin = MIN(y,inputYMin);
            inputXMax = MAX(x,inputXMax);
            inputXMin = MIN(x,inputXMin);
        } else if(IS_HIDDEN_UNIT(unitPtr) && UNIT_IN_USE(unitPtr)) {
            NoOfHiddenUnits++;
            hiddenYMax = MAX(y,hiddenYMax);
            hiddenYMin = MIN(y,hiddenYMin);
            hiddenXMax = MAX(x,hiddenXMax);
            hiddenXMin = MIN(x,hiddenXMin);
        } else if(IS_OUTPUT_UNIT(unitPtr) && UNIT_IN_USE(unitPtr)) {
            NoOfOutputUnits++;
            outputYMax = MAX(y,outputYMax);
            outputYMin = MIN(y,outputYMin);
            outputXMax = MAX(x,outputXMax);
            outputXMin = MIN(x,outputXMin);
        }
    }
...
void cc_redisplay(void) {
    ...
    if (MaxUnitsInLayer <= cc_display_mode) {
        ...
        case(Y_MAX_MODE1) :
            cc_display_mode = Y_MAX_MODE2;
            break;
        case(Y_MAX_MODE2) :
            cc_display_mode = Y_MAX_MODE3;
            break;
        case(Y_MAX_MODE3) :
            cc_display_mode = Y_MAX_MODE4;
            break;
        case(Y_MAX_MODE4) :
            cc_display_mode = Y_MAX_MODE5;
            break;
