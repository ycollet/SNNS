...
static int  kr_searchOutputConnection(struct Unit *start_unit_ptr,
    ...
    for(unit_ptr = start_unit_ptr; unit_ptr <= unit_array + MaxUnitNo; unit_ptr++)
        if UNIT_IN_USE( unit_ptr ) {
            ...
            } else if UNIT_HAS_SITES( unit_ptr ) {
...
bool  kr_areConnected(int source_unit_no, int target_unit_no, FlintType *weight) {
    ...
    } else if UNIT_HAS_SITES( target_unit_ptr )
...
bool  kr_isConnected(int source_unit_no, FlintType *weight) {
    ...
    else if UNIT_HAS_SITES( unitPtr )
...
static krui_err  kr_copyInputLinks(struct Unit *source_unit_ptr,
    ...
    } else if UNIT_HAS_SITES( source_unit_ptr )
...
int  kr_flags2TType(int flags) {
    ...
    switch (flags) {
    case UFLAG_TTYP_UNKN:
        return( UNKNOWN );
    case UFLAG_TTYP_IN  :
        return( INPUT );
    case UFLAG_TTYP_OUT :
        return( OUTPUT );
    case UFLAG_TTYP_DUAL:
        return( DUAL );
    case UFLAG_TTYP_HIDD:
        return( HIDDEN );
    case UFLAG_TTYP_SPEC:
        return( SPECIAL );
    case UFLAG_TTYP_SPEC_I:
        return (SPECIAL_I) ;
    case UFLAG_TTYP_SPEC_O:
        return (SPECIAL_O) ;
    case UFLAG_TTYP_SPEC_H:
        return (SPECIAL_H) ;
    case UFLAG_TTYP_SPEC_D:
        return (SPECIAL_D) ;
...
int  kr_TType2Flags(int ttype) {
    ...
    switch (ttype) {
    case UNKNOWN:
        return( UFLAG_TTYP_UNKN );
    case INPUT	:
        return( UFLAG_TTYP_IN );
    case OUTPUT :
        return( UFLAG_TTYP_OUT );
    case DUAL	:
        return( UFLAG_TTYP_DUAL );
    case HIDDEN :
        return( UFLAG_TTYP_HIDD );
    case SPECIAL:
        return( UFLAG_TTYP_SPEC );
    case SPECIAL_I:
        return (UFLAG_TTYP_SPEC_I) ;
    case SPECIAL_O:
        return (UFLAG_TTYP_SPEC_O) ;
    case SPECIAL_H:
        return (UFLAG_TTYP_SPEC_H) ;
    case SPECIAL_D:
        return (UFLAG_TTYP_SPEC_D) ;
    case SPECIAL_X:
        return (UFLAG_TTYP_SPEC_X) ;
    case N_SPECIAL_X:
        return (UFLAG_TTYP_N_SPEC_X) ;
    default:
        KernelErrorCode = KRERR_TTYPE;
        /*  return( KernelErrorCode );  */
        return( -1 );
