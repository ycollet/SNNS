...
static void computeExtents(GW gw) {
    ...
    switch(gw->kind) {
    case proj2D:
        gw->heightPerVector = gw->widthPerVector = gw->curWidth;
        break;
    case poly:
    case histo:
        gw->widthPerVector = ndims * gw->curWidth +
                             (2 * X_DIST_FROM_EDGE) + 1;
        gw->heightPerVector = POLY_HISTO_HEIGHT;
        break;
    case clrMat:
    case grayMat:
        gw->widthPerVector = (long) gw->arg2 * gw->curWidth +
                             (2 * LINE_WIDTH);
...
static Boolean notPrintableGW(GW gw) {
    ...
    switch(gw->kind) {
proj2D:
        if(gw->arg1 > ndims || gw->arg2 > ndims) {
