        ...
        if(Val->ytype != Undef) {
            ...
            if(Val->ytype ==
                ...
                ER_StackError("type mismatch: initial and end value for loop variable\n",
                              Serious, nst, nst, nst);
                ConditionValue = FALSE;
                AbortLoop = TRUE;           /* do not execute loop body */
...
ArrayElemType Val;                                       /* new value for variable */
{
    switch(Var->Type) {
    case XINT:
        Var->Value->intval = *(Val.aint);
        break;                         /* simple integer */
    case XFLOAT:
        Var->Value->floatval = *(Val.afloat);
        break;                       /* simple float */
    case STRING:
    case LONGSTRING:
    case SIMPLESTRING:
        Var->Value->string = *(Val.astring);
        break;                        /* string types */
    case UNIT:
        Var->Value->unit = *(Val.aunit);
        break;                             /* single unit */
    case STRUCTURE:
        Var->Value->structure = *(Val.astruct);
        break;
    case ArrStruct:
    case ArrUnit:
...
SymbolType Type;
{
    switch(Type) {
    case XINT:
        AVal->aint = (Val.aint + Inc);
        break;
    case XFLOAT:
        AVal->afloat = (Val.afloat + Inc);
        break;
    case STRING:
    case LONGSTRING:
    case SIMPLESTRING:
        AVal->astring = (Val.astring + Inc);
        break;
    case UNIT:
        AVal->aunit = (Val.aunit + Inc);
        break;
    case STRUCTURE:
        AVal->astruct = (Val.astruct + Inc);
        break;
    case ArrStruct:
    case ArrUnit:
    case ArrInt:                          /* array types - same action for all base types */
