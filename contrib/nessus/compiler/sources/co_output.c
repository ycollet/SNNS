...
int Ctr;                                     /* index of current unit in structure */
{
    ...
    if(S->UnitDef->FType)                           /* there's a named type for this unit */
        ...
        else if(S->ExplUnits && *(S->ExplUnits+Ctr) ) /* THIS unit has been defined explicitly */
...
UnitType *S;                                     /* pointer to structure node */
{
    ...
    else if(S->FParam.VarFT->ActFunc)     /* activation function cannot be defined */
...
UnitType *S;                                     /* pointer to structure node */
{
    ...
    else if(S->FParam.VarFT->OutFunc)         /* output function cannot be defined */
    ...
    for(SNo = Structure->Begin, LineEntries = 0, First = TRUE;
        ...
        if(SNo != UnitNo)                            /* a unit cannot be connected to itself */
            if(First) {
                ...
                LineEntries ++;
                First = FALSE;
            } else if(LineEntries < SourPerLine) {   /* print next source into same output line */
