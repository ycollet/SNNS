...

{
    ...
    if (*argc < 7)
        return 0;
    TD_LINK_source_plane          = more_data(&rest, argv);
    RETURN_IF_FAULT(rest);
    TD_LINK_source_cluster_x      = more_data(&rest, argv);
    RETURN_IF_FAULT(rest);
    TD_LINK_source_cluster_y      = 1;
    TD_LINK_source_cluster_width  = more_data(&rest, argv);
    RETURN_IF_FAULT(rest);
    TD_LINK_source_cluster_height = more_data(&rest, argv);
    RETURN_IF_FAULT(rest);

    TD_LINK_target_plane          = more_data(&rest, argv);
    RETURN_IF_FAULT(rest);
    TD_LINK_target_cluster_x      = more_data(&rest, argv);
    RETURN_IF_FAULT(rest);
    TD_LINK_target_cluster_y      = 1;
    TD_LINK_target_cluster_width  = more_data(&rest, argv);
    RETURN_IF_FAULT(rest);
