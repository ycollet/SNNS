...
void checkErr(int errCode) {
    ...
    case CANT_OPEN   :
        ...
        break;
    case NO_CPN      :
        printf("net is not a CounterPropagation network\n");
        break;
    case NO_TDNN     :
        printf("net is not a Time Delay Neural Network\n");
        break;
    case ILLEGAL_CYCLES :
        printf("net contains illegal cycles\n");
        break;
    case WRONG_ACT_FUNC :
        ;
        break;
    case NOT_SUPPORTED  :
        printf("not supported network type\n");
        break;
    default             :
...
#define NAME_LENGTH 11
    ...
    for(nr = 0; nr < NoOfLayers; nr++) {
        ...
        case INPUT  :
            sprintf(layer->name, "Input");
            break;

        case OUTPUT :
            sprintf(layer->name, "Output%d", ++ocounter);
            break;

        case HIDDEN :
            sprintf(layer->name, "Hidden%d", ++hcounter);
            break;

        default     :
            sprintf(layer->name, "Special%d", ++scounter);
        }
...
int writeTdnnNet(pLayer globalLayers, pUnit globalUnits,
    ...
    for(nr = 0; nr < NoOfLayers; nr ++) {
        ...
        } else if (addList(FunctionList, layer->ActFunc) ) return(MEM_ERR);
    }
