    ...
    if (na.graphtype == NA_GRAPH_TE) {
        ...
        case NA_ERROR_LIN :
            if (na.ave)
                ...
                strcpy  (buf2, "vertical   : error (lin)") ;
            break ;
        case NA_ERROR_SQR :
            if (na.ave)
                strcpy  (buf2, "vertical   : error (sqr ave)") ;
            else
                strcpy  (buf2, "vertical   : error (sqr)") ;
            break ;
        case NA_ERROR_SU  :
...

{
    ...
    n = 0 ;
    XtSetArg    (args[n], XtNwidth, &w_width ) ;
    n++ ;
    XtSetArg    (args[n], XtNheight, &w_height) ;
    n++ ;
    ...
    n = 0 ;
    XtSetArg    (args[n], XtNhorizDistance, &s_hor) ;
    n++ ;
    XtSetArg    (args[n], XtNvertDistance, &s_ver) ;
    n++ ;
    XtGetValues (w_Screen, args, n) ;

    n = 0 ;
    XtSetArg    (args[n], XtNwidth, &b_width) ;
    n++ ;
    XtSetArg    (args[n], XtNhorizDistance, &b_hor  ) ;
    n++ ;
    XtSetArg    (args[n], XtNvertDistance, &b_ver  ) ;
    n++ ;
...

{
    ...
    n = 0 ;
    XtSetArg (args[n], XtNheight, (Dimension) na.window.h) ;
    n++ ;
    XtSetArg (args[n], XtNwidth, (Dimension) na.window.w) ;
    n++ ;
    ...
    n = 0 ;
    XtSetArg (args[n], XtNheight, (Dimension) na.pixmap.h ) ;
    n++ ;
    XtSetArg (args[n], XtNwidth, (Dimension) na.pixmap.w ) ;
    n++ ;
    ...
    XtSetArg (args[n], XtNfromVert, NULL        ) ;
    n++ ;
    XtSetArg (args[n], XtNfromHoriz, w_Screen    ) ;
    n++ ;
    XtSetArg (args[n], XtNleft, XtChainLeft ) ;
    n++ ;
    XtSetArg (args[n], XtNright, XtChainLeft ) ;
    ...
    XtSetArg (args[n], XtNborderWidth, 0           ) ;
    n++ ;
    XtSetArg (args[n], XtNvertDistance, 0           ) ;
    n++ ;
    ...
    if (na_FirstCall) {
        n = 0 ;
        XtSetArg    (args[n], XtNwidth, &na_Width ) ;
        n++ ;
        XtSetArg    (args[n], XtNheight, &na_Height) ;
        n++ ;
        XtGetValues (w_NetworkAnalyzer, args, n) ;
    }

    n = 0 ;
    XtSetArg    (args[n], XtNminWidth, na_Width ) ;
    n++ ;
    XtSetArg    (args[n], XtNminHeight, na_Height) ;
    n++ ;
...

{
    ...
    if (w_OldSetup == NULL) {
        ...
        n = 0;
        XtSetArg (args[n], XtNx, setup_x      ) ;
        n++ ;
        XtSetArg (args[n], XtNy, setup_y      ) ;
        n++ ;
...

{
    ...
    n = 0 ;
    XtSetArg    (args[n], XtNminWidth,  width ) ;
    n++ ;
    XtSetArg    (args[n], XtNminHeight,  height) ;
    n++ ;
    XtSetArg    (args[n], XtNmaxWidth,  width ) ;
    n++ ;
    XtSetArg    (args[n], XtNmaxHeight,  height) ;
    n++ ;
