...

{
    ...
    case UI_FILE_NET:
        ...
        break;
    case UI_FILE_PAT:
        ui_xSetToggleState (patToggle, TRUE);
        ui_xSetString(fileBox, ui_filenamePAT);
        strcpy (extMask, ".pat");
        break;
    case UI_FILE_RES:
        ui_xSetToggleState (resToggle, TRUE);
        ui_xSetString(fileBox, ui_filenameRES);
        strcpy (extMask, ".res");
        break;
    case UI_FILE_CFG:
        ui_xSetToggleState (cfgToggle, TRUE);
        ui_xSetString(fileBox, ui_filenameCFG);
        strcpy (extMask, ".cfg");
        break;
    case UI_FILE_TXT:
        ui_xSetToggleState (txtToggle, TRUE);
    ...
    ui_readDirectory ();
    n = 0;
    XtSetArg(args[n], XtNstring, sortedDirEntries);
    n++;
            ...
            if (start != end) {
                ...
                if (found > 0) {
                    ...
                    if (nlStarts AND nlEnds) {
                        ...
                        if (realName[0] != '[') {
                            n = 0;
                            XtSetArg (args[n], XtNstring, realName);
                            n++;
...

{
    ...
    n++;
    XtSetArg(args[n], XtNfromVert, ui_path);
    n++;
