...
static void ui_init_monochromePalette(void) {
    ...
    if (NOT (XAllocColor(ui_display, ui_colormap, &color))) {
        ...
        XtSetArg(args[n], XtNcolormap, ui_colormap);
        n++;
        XtSetArg(args[n], XtNbackground, WhitePixel(ui_display,ui_screen));
        n++;
        XtSetArg(args[n], XtNforeground, BlackPixel(ui_display,ui_screen));
        n++;
        ...
        for (i=0; i <= 2 * ui_col_steps; i++) {
            ...
            if (NOT (XAllocColor(ui_display, ui_colormap, &color))) {
                ...
                XtSetArg(args[n], XtNbackground,
                         WhitePixel(ui_display,ui_screen));
                n++;
                XtSetArg(args[n],
                         XtNforeground, BlackPixel(ui_display,ui_screen));
                n++;
                XtSetValues(ui_toplevel, args, n);
