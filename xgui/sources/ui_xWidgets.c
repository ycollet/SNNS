...
void ui_xSetResize(Widget widget,Boolean state) {
    ...
    n++;
    XtSetArg(args[n], XtNresizable, state);
    n++;
...
void ui_xSetLabel(Widget widget,char *newLabel) {
    ...
    n = 0;
    XtSetArg(args[n], XtNlabel, newLabel);
    n++;
    XtSetValues(widget, args, n);
}
...
void ui_xSetString(Widget widget,char *contents) {
    ...
    n = 0;
    XtSetArg(args[n], XtNstring,editBuf);
    n++;
    XtSetArg(args[n], XtNinsertPosition, lenContents);
    n++;
    XtSetValues(widget, args, n);
}
...
void ui_xStringFromAsciiWidget(Widget widget,char *stringVar,int maxLen) {
    ...
    n = 0;
    XtSetArg(args[n], XtNtextSource, &asciiSrc);
    n++;
...
Bool ui_xGetToggleState(Widget w) {
    ...
    n=0;
    XtSetArg(args[n], XtNstate, &state);
    n++;
...
void ui_xSetToggleState(Widget w,Bool state) {
    ...
    n=0;
    XtSetArg(args[n], XtNstate, (Boolean) state);
    n++;
...
Widget   ui_xCreateLabelItem(char *name,Widget parent,Dimension width,Widget left,Widget top) {
    ...
    n++;
    if (width != 0) {
        XtSetArg(args[n], XtNwidth, (XtArgVal) width);
        n++;
        ...
        n++;
        XtSetArg(args[n], XtNresize,    TRUE);
        n++;
    }
...
Widget   ui_xCreateFrameLabelItem(char *name,Widget parent,Dimension width,Widget left,Widget top) {
    ...
    XtSetArg(args[n], XtNjustify, XtJustifyLeft);
    n++;
    XtSetArg(args[n], XtNinternalHeight, 3);
    n++;
    XtSetArg(args[n], XtNinternalWidth, 4);
    n++;
    XtSetArg(args[n], XtNfromHoriz, left);
    n++;
    XtSetArg(args[n], XtNfromVert, top);
...
Widget  ui_xCreateDialogItem(char *name,Widget parent,char *contents,Dimension  width,Widget left,Widget top) {
    ...
    if (width != 0) {
        ...
        XtSetArg(args[n], XtNwidth, 8*lenContents + 20);
        n++;
        XtSetArg(args[n], XtNresizable, True);
        n++;
        XtSetArg(args[n], XtNresize, XawtextResizeWidth);
...
Widget  ui_xCreateScrollbarItem(char *name,Widget parent,Dimension length,float topOfThumb,Widget left,Widget top) {
    ...
    XtSetArg(args[n], XtNborderWidth, 1);
    n++;
    XtSetArg(args[n], XtNlength, length);
    n++;
    XtSetArg(args[n], XtNthickness, 15);
    n++;
...
static void ui_moveleft(Widget w,Widget pannel,caddr_t call_data) {
    ...
    XtGetValues (w, args, 1);
    n=0;
    XtSetArg    (args[n], XtNx, &nx);
    n++;
    XtSetArg    (args[n], XtNy, &ny);
    n++;
    XtSetArg    (args[n], XtNinsertPosition, &pos);
    n++;
...
#define WINDOW_HEADER 30
    ...
    XtRealizeWidget(w);
    n = 0;
    XtSetArg(args[n], XtNx, &WindowX);
    n++;
    XtSetArg(args[n], XtNy, &WindowY);
    n++;
    XtSetArg(args[n], XtNwidth, &WindowWidth);
    n++;
    XtSetArg(args[n], XtNheight, &WindowHeight);
    n++;
    XtGetValues(w, args, n);
...

{
    ...
    n=0;
    XtSetArg (args[n], XtNwidth, &x);
    n++;
    XtSetArg (args[n], XtNheight, &y);
    n++;
    XtGetValues  (w, args,  n);
    n=0;
    XtSetArg (args[n], XtNmaxWidth, x);
    n++;
    XtSetArg (args[n], XtNmaxHeight, y);
    n++;
    XtSetArg (args[n], XtNminWidth, x);
    n++;
    XtSetArg (args[n], XtNminHeight, y);
    n++;
    XtSetValues  (w, args,  n);
}
/* end of file */
